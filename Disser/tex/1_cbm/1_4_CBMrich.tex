...
Процедура восстановления события в CBM RICH детекторе схематично показана на~\figref{fig:StsRichReco}, подробнее см.~\cite{RICHreco}.
...
%Реализация данного алгоритма была специально адаптирована для данных пучковых тестов, в которых ожидается одно кольцо на событие. Данный алгоритм реализован в классе \classname{CbmRichProtRingFinderHoughImpl}, унаследованном от \classname{CbmRichProtRingFinderHough} и далее от \classname{CbmRichRingFinder}.
...
% На~\figref{fig:RICHeff}, \figref{fig:InvMassSpectra} и табл.~\ref{tabl:RICHchar} показаны
...
Вследствие большой множественности в центральных столкновениях в одном событий в RICH регистрируется до 65~колец.
...
Для определения характеристик CBM RICH с помощью моделирования в каждое событие к частицам UrQMD были подмешаны по~$5e^{+}$ и~$5e^{-}$ из точки взаимодействия, имеющих поперечный импульс, разыгранный в соответствии с равномерным распределением в диапазоне от~0 до~3~\GeVoverC{}, и направлением в пределах геометрического аксептанса. В моделировании при энергии пучка~8~\GeVperNucl{} был исключён детектор TRD.

% In order to enhance statistics and investigate the RICH response to primary electrons, 5 e+ and 5 e- were embedded into each UrQMD event at the primary vertex with uniform distributions of transverse momentum in 0 < p T < 3 GeV/c, azimuth angle 0 < φ <360 and polar angle 2.5 < θ < 25.
% The detector setup which is described in Section 3.1 was used for simulations.
% However for simulations at 8 AGeV beam energy the TRD detector was excluded.

% The performance of the algorithms was obtained using information on Monte Carlo input. During the efficiency calculation the level of the correspondence between found and simulated rings is estimated. A ring is defined as correctly found if it has more than 60\% of hits from one Monte-Carlo ring, otherwise a ring is defined as ghost ring.

На~\figref{fig:RICHeff} показана эффективность реконструкции колец, которая определяется как отношение $N_{rec} / N_{acc}$, где $N_{rec}$ --- количестве верно найденных колец после реконструкции, а $N_{acc}$ --- количество колец, находящихся в аксептансе RICH, которые возможно реконструировать, т.е. колец, имеющих минимум 7~зарегистрированных хитов в RICH.


% The ring reconstruction efficiency is defined as $Efficiency = N_{reconstructed} / N_{accepted}$. $N_reconstructed$ is the number of correctly found rings after reconstruction, $N_{accepted}$ is the number of reconstructable rings lying in the RICH acceptance, i.e. rings which have at least 7 detected hits in the RICH.
% Results in this section are shown for two sets of rings: rings from primary e± with at least 7 hits and rings from for primary ``reference'' e± which have more than 15 hits in the ring and a momentum larger than 1 GeV/c.

% The electron identification efficiency is defined as the number of electrons which were correctly reconstructed, matched and identified in STS+RICH(+TRD+TOF) detectors divided by the number of electrons in the STS, RICH, TRD and TOF detector acceptance. The acceptance is defined by requiring at least 4 consecutive STS points, at least 7 RICH hits, at least 8 TRD points and one TOF hit. Since for 8 AGeV the TRD is not used the detector acceptance calculation also does not include the TRD for this energy. The pion suppression factor is defined as the number of pions which were reconstructed in the STS and have a track projection lying in the RICH acceptance divided by the number of pions identified as electrons.
...
% Left: electron identification efficiency in dependence on momentum. Right: pion suppression factor in dependence on momentum; both calculated for 25 AGeV beam energy. Different colours represent different reconstruction steps and detectors which are included in the electron identification procedure: RICH (red), RICH+TRD (blue) and RICH+TRD+TOF (green). Horizontal lines represent numbers integrated over momentum.

% \todo проверить ссылки на таблицы

На~\figref{fig:InvMassSpectra} показаны смоделированные спектры инвариантных масс диэлектронов при распаде лёгких векторных мезонов с учётом разнообразных фонов, а в таблице~\ref{tabl:RICHchar} собрана информация о фонах и отношении сингал-фон при реконструкции этих частиц с помощью CBM RICH.
